...
export const getVitalStatus=(type,val)=>{
  ...
  switch(type){
    ...
    case"hr":if(val>110||val<45)return{label:"Anormal",icon:""};if(val>100)return{label:"Eleve",icon:""};return{label:"Normal",icon:""};
    case"temp":if(val>=39)return{label:"Fievre",icon:""};if(val>=38)return{label:"Subfebrile",icon:""};return{label:"Normal",icon:""};
    case"resp":if(val>25)return{label:"Critique",icon:""};if(val>20)return{label:"Tachypnee",icon:""};return{label:"Normal",icon:""};
